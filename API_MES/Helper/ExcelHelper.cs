...
namespace API_MES.Helper
{
    ...
    public class ExcelHelper
    {
        ...
        public ActionResult ExcelDownload<T>(IEnumerable<T> list, IEnumerable<ExcelGridModel> columnList, string fileName)
        {
            ...
            var streams = ExcelHelpers.GetDataTableToMemory(rowData, columnList.Select(i => i.label), true);
            //var streams = ExcelHelpers.OutAsToExcelToMemory(rowData);
            //var stream = ExportMemoryStream(rowData, excelConfig);
            //return new FileStreamResult(streams, MIMEType.xlsx) { FileDownloadName = JointXls(fileName) };
            return new FileStreamResult(streams, MIMEType.xlsx) { FileDownloadName = JointXls(fileName) };
            
        }
        public MemoryStream ExcelMemory<T>(IEnumerable<T> list, IEnumerable<ExcelGridModel> columnList, string fileName)
        {
            var excelConfig = ConvertExcelGridModelToConfig(columnList, fileName);
            DataTable rowData = list.ToDataTable(columnList.Select(i => i.name));
            MemoryStream streams = ExcelHelpers.GetDataTableToMemory(rowData, columnList.Select(i => i.label), true);
            //var streams = ExcelHelpers.OutAsToExcelToMemory(rowData);
            //var stream = ExportMemoryStream(rowData, excelConfig);
            return streams;
        ...
        public ActionResult ExcelDownload(DataTable dtSource, IEnumerable<ExcelGridModel> columnList, string fileName)
        {
            ...
            return new FileStreamResult(stream, MIMEType.xlsx) { FileDownloadName = JointXls(fileName) };
        }
